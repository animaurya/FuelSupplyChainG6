...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	return fabric_client.getUserContext('depotadmin', true);
}).then((user_from_store) => {
	if (user_from_store && user_from_store.isEnrolled()) {
		console.log('Successfully loaded depotadmin from persistence');
		member_user = user_from_store;
	} else {
		throw new Error('Failed to get depotadmin.... run registerUser.js');
	}
	...
	var request = {chaincodeId: 'mycc',
		fcn: 'addFuel',
		args: [req.body.owner, req.body.volume, req.body.temperature, req.body.density],
		chainId: 'mychannel',
		txId: tx_id};
	...
	if (results && results[0] && results[0].status === 'SUCCESS') {
		...
		res.send({code:"500", message: "add fuel failed."});
	}

	if(results && results[1] && results[1].event_status === 'VALID') {
		console.log('Successfully committed the change to the ledger by the peer');
		res.send({code:"200", message: "fuel  added successsfully."});
	} else {
